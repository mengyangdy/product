...
import { BASE_URL, FILE_URL } from '@/configs'
...
async function fetchData() {
  ...
  fetchRequest(`${BASE_URL}/data/videos.md`).then(r => {
    r.json().then(async v => {
...
export async function startMock() {
  ...
  mock.onGet(/video\/comments/).reply(async config => {
    ...
    const r2 = await fetchRequest(`${FILE_URL}/comments/video_id_${id}.md`)
  ...
  mock.onGet(/video\/my/).reply(async config => {
    ...
    if (!userVideos.length) {
      ...
      const r = await fetchRequest(
        `${BASE_URL}/data/user_video_list/user-12345xiaolaohu.md`
      )
  ...
  mock.onGet(/user\/video_list/).reply(async config => {
    ...
    const r2 = await fetchRequest(`${FILE_URL}/user_video_list/user-${id}.md`)
    const v = await r2.json()
    if (v) {
      return [200, { data: v, code: 200 }]
    }
  ...
  mock.onGet(/user\/panel/).reply(async () => {
    const r2 = await fetchRequest(`${BASE_URL}/data/users.md`)
    const v = await r2.json()
  ...
  mock.onGet(/post\/recommended/).reply(async config => {
    ...
    if (!allRecommendPosts.length) {
      const r = await fetchRequest(`${BASE_URL}/data/posts.md`)
  ...
  mock.onGet(/shop\/recommended/).reply(async config => {
    ...
    const r2 = await fetchRequest(`${BASE_URL}/data/goods.md`)
    const v = await r2.json()
    return [
      200,
      {
